...
// Module implements the page analytics module.
type Module struct {
	module.BaseModule
...
// New creates a new page analytics module.
func New() *Module {
	return &Module{
		BaseModule: module.NewBaseModule(
			"page_analytics",
			"1.0.0",
			"Built-in server-side page analytics with privacy focus",
		),
...
func (m *Module) Init(ctx *module.ModuleContext) error {
	...
	if err != nil {
		ctx.Logger.Warn("failed to load page_analytics settings, using defaults", "error", err)
		settings = &Settings{
			Enabled:           true,
			RetentionDays:     365,
	...
	ctx.Logger.Info("Page Analytics module initialized",
...
func (m *Module) Shutdown() error {
	...
	if m.ctx != nil {
		m.ctx.Logger.Info("Page Analytics module shutting down")
	}
	return nil
}
...
func (m *Module) RegisterAdminRoutes(r chi.Router) {
	r.Get("/page-analytics", m.handleDashboard)
	r.Get("/page-analytics/api/stats", m.handleAPIStats)
	r.Get("/page-analytics/api/realtime", m.handleRealtime)
	r.Post("/page-analytics/settings", m.handleSaveSettings)
}
