// Package analytics provides a statistics tracking module for oCMS.
...
// Module implements the module.Module interface for the analytics module.
type Module struct {
	module.BaseModule
...
// New creates a new instance of the analytics module.
func New() *Module {
	return &Module{
		BaseModule: module.NewBaseModule(
			"analytics",
			"1.0.0",
			"Statistics tracking for GA4, GTM, and Matomo",
		),
	}
}
...
func (m *Module) Init(ctx *module.ModuleContext) error {
	...
	if err != nil {
		ctx.Logger.Warn("failed to load analytics settings, using defaults", "error", err)
		settings = &Settings{}
	}
	m.settings = settings

	m.ctx.Logger.Info("Analytics module initialized",
...
func (m *Module) Shutdown() error {
	if m.ctx != nil {
		m.ctx.Logger.Info("Analytics module shutting down")
	}
	return nil
}
...
func (m *Module) RegisterAdminRoutes(r chi.Router) {
	r.Get("/analytics", m.handleDashboard)
	r.Post("/analytics", m.handleSaveSettings)
}

// TemplateFuncs returns template functions provided by the module.
func (m *Module) TemplateFuncs() template.FuncMap {
	return template.FuncMap{
		// analyticsHead returns tracking scripts for the <head> section
		"analyticsHead": func() template.HTML {
			return m.renderHeadScripts()
		},
		// analyticsBody returns tracking scripts for the end of <body>
		"analyticsBody": func() template.HTML {
