...
func TestModuleNew(t *testing.T) {
	...
	if m.Name() != "analytics" {
		t.Errorf("Name() = %q, want analytics", m.Name())
	}
	if m.Version() != "1.0.0" {
...
func TestModuleAdminURL(t *testing.T) {
	...
	if m.AdminURL() != "/admin/analytics" {
		t.Errorf("AdminURL() = %q, want /admin/analytics", m.AdminURL())
	}
}
...
func TestModuleTemplateFuncs(t *testing.T) {
	...
	// Check analyticsHead function exists
	if _, ok := funcs["analyticsHead"]; !ok {
		t.Error("analyticsHead function not found")
	}

	// Check analyticsBody function exists
	if _, ok := funcs["analyticsBody"]; !ok {
		t.Error("analyticsBody function not found")
	}
}
