...
// handleDashboard handles GET /admin/analytics - shows the analytics settings page.
func (m *Module) handleDashboard(w http.ResponseWriter, r *http.Request) {
	user := middleware.GetUser(r)
	lang := m.ctx.Render.GetAdminLang(r)

	if err := m.ctx.Render.Render(w, r, "admin/module_analytics", render.TemplateData{
		Title: i18n.T(lang, "analytics.title"),
		User:  user,
		Data:  m.settings,
		Breadcrumbs: []render.Breadcrumb{
			{Label: i18n.T(lang, "nav.dashboard"), URL: "/admin"},
			{Label: i18n.T(lang, "nav.modules"), URL: "/admin/modules"},
			{Label: i18n.T(lang, "analytics.title"), URL: "/admin/analytics", Active: true},
		},
	}); err != nil {
...
// handleSaveSettings handles POST /admin/analytics - saves analytics settings.
func (m *Module) handleSaveSettings(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseForm(); err != nil {
		...
		m.ctx.Render.SetFlash(r, i18n.T(lang, "analytics.error_parse_form"), "error")
	...
	if newSettings.GA4Enabled && newSettings.GA4MeasurementID == "" {
		validationErrors = append(validationErrors, i18n.T(lang, "analytics.error_ga4_id_required"))
	}

	if newSettings.GTMEnabled && newSettings.GTMContainerID == "" {
		validationErrors = append(validationErrors, i18n.T(lang, "analytics.error_gtm_id_required"))
	}

	if newSettings.MatomoEnabled {
		if newSettings.MatomoURL == "" {
			validationErrors = append(validationErrors, i18n.T(lang, "analytics.error_matomo_url_required"))
		}
		if newSettings.MatomoSiteID == "" {
			validationErrors = append(validationErrors, i18n.T(lang, "analytics.error_matomo_site_id_required"))
		}
	}
	...
	if err := saveSettings(m.ctx.DB, newSettings); err != nil {
		m.ctx.Logger.Error("failed to save analytics settings", "error", err)
		m.ctx.Render.SetFlash(r, i18n.T(lang, "analytics.error_save"), "error")
		http.Redirect(w, r, "/admin/analytics", http.StatusSeeOther)
		return
	}
	...
	m.ctx.Logger.Info("analytics settings updated",
		"user", user.Email,
	...
	m.ctx.Render.SetFlash(r, i18n.T(lang, "analytics.success_save"), "success")
	http.Redirect(w, r, "/admin/analytics", http.StatusSeeOther)
}
