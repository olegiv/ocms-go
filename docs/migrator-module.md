...
`authors` is optional — an install stripped down to a content archive may not have it, and the import continues without it, recording a job notice and falling back to `users` for every byline. Only `ER_NO_SUCH_TABLE` counts as absence: a missing `SELECT` grant or a dropped connection is reported as an error, because an empty read otherwise looks exactly like a site that never had administrators. Every other table in the list is required.
