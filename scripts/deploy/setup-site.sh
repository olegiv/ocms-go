...
echo "  3. Bootstrap the first admin account:"
echo "               cd $INSTANCE_DIR"
echo "               sudo -u $SYSTEM_USER env OCMS_ENV=development OCMS_DO_SEED=true /opt/ocms/bin/ocms"
echo "               Login at https://$DOMAIN/admin/ with admin@example.com / changeme1234"
echo "               Rotate credentials, then stop the foreground process with Ctrl+C"
echo "  4. Enable:   sudo systemctl enable --now ocms@$SITE_ID"
echo "  5. Logs:     sudo journalctl -u ocms@$SITE_ID -f"
