...
const getPublishedTagUsageCounts = `-- name: GetPublishedTagUsageCounts :many
...
type GetPublishedTagUsageCountsParams struct {
	Limit  int64 `json:"limit"`
	Offset int64 `json:"offset"`
}

type GetPublishedTagUsageCountsRow struct {
	ID           int64     `json:"id"`
	Name         string    `json:"name"`
	Slug         string    `json:"slug"`
	LanguageCode string    `json:"language_code"`
...
func (q *Queries) GetPublishedTagUsageCounts(ctx context.Context, arg GetPublishedTagUsageCountsParams) ([]GetPublishedTagUsageCountsRow, error) {
	rows, err := q.db.QueryContext(ctx, getPublishedTagUsageCounts, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetPublishedTagUsageCountsRow{}
	for rows.Next() {
		var i GetPublishedTagUsageCountsRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Slug,
...
const getTagNamesForPublishedPages = `-- name: GetTagNamesForPublishedPages :many
SELECT pt.page_id, t.name
FROM page_tags pt
INNER JOIN tags t ON t.id = pt.tag_id
INNER JOIN pages p ON p.id = pt.page_id AND p.status = 'published'
...
type GetTagNamesForPublishedPagesRow struct {
	PageID int64  `json:"page_id"`
	Name   string `json:"name"`
}

func (q *Queries) GetTagNamesForPublishedPages(ctx context.Context) ([]GetTagNamesForPublishedPagesRow, error) {
	rows, err := q.db.QueryContext(ctx, getTagNamesForPublishedPages)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetTagNamesForPublishedPagesRow{}
	for rows.Next() {
		var i GetTagNamesForPublishedPagesRow
		if err := rows.Scan(&i.PageID, &i.Name); err != nil {
			return nil, err
		}
		items = append(items, i)
