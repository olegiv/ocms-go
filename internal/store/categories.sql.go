...
const getCategoryNamesForPublishedPages = `-- name: GetCategoryNamesForPublishedPages :many
SELECT pc.page_id, c.name
FROM page_categories pc
INNER JOIN categories c ON c.id = pc.category_id
...
type GetCategoryNamesForPublishedPagesRow struct {
	PageID int64  `json:"page_id"`
	Name   string `json:"name"`
}

func (q *Queries) GetCategoryNamesForPublishedPages(ctx context.Context) ([]GetCategoryNamesForPublishedPagesRow, error) {
	rows, err := q.db.QueryContext(ctx, getCategoryNamesForPublishedPages)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetCategoryNamesForPublishedPagesRow{}
	for rows.Next() {
		var i GetCategoryNamesForPublishedPagesRow
		if err := rows.Scan(&i.PageID, &i.Name); err != nil {
			return nil, err
		}
		items = append(items, i)
...
const getPublishedCategoryUsageCounts = `-- name: GetPublishedCategoryUsageCounts :many
SELECT c.id, c.name, c.slug, c.description, c.parent_id, c.position, c.created_at, c.updated_at, c.language_code, COUNT(p.id) as usage_count
FROM categories c
INNER JOIN page_categories pc ON pc.category_id = c.id
INNER JOIN pages p ON p.id = pc.page_id AND p.status = 'published'
...
type GetPublishedCategoryUsageCountsRow struct {
	ID           int64          `json:"id"`
	Name         string         `json:"name"`
	Slug         string         `json:"slug"`
	Description  sql.NullString `json:"description"`
...
func (q *Queries) GetPublishedCategoryUsageCounts(ctx context.Context) ([]GetPublishedCategoryUsageCountsRow, error) {
	rows, err := q.db.QueryContext(ctx, getPublishedCategoryUsageCounts)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetPublishedCategoryUsageCountsRow{}
	for rows.Next() {
		var i GetPublishedCategoryUsageCountsRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Slug,
