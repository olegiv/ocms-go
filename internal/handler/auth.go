...
func (h *AuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if !valid {
		...
		_ = h.eventService.LogAuthEvent(r.Context(), model.EventLevelWarning, "Login failed: invalid password", &user.ID, map[string]any{"email": email})
...
func (h *AuthHandler) Logout(w http.ResponseWriter, r *http.Request) {
	...
	if userID > 0 {
		_ = h.eventService.LogAuthEvent(r.Context(), model.EventLevelInfo, "User logged out", &userID, nil)
	}
